...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${tptBackendUrl}/admin/teams/overview`;
    ...
    if (!response.ok) {
      console.error(
        `Failed to fetch teams overview: ${response.status} ${response.statusText}`
      );
      return NextResponse.json(
        { error: "errors.fetchTeamsOverviewError" },
