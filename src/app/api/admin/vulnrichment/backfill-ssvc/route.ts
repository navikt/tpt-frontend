...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = `${tptBackendUrl}/admin/vulnrichment/backfill-ssvc`;
    ...
    if (!response.ok) {
      console.error(
        `Failed to trigger SSVC backfill: ${response.status} ${response.statusText}`
      );
      return NextResponse.json(
        { error: "errors.triggerSsvcBackfillError" },
